...
public final class CakeReflection {
    ...
    public static Class<?> findClass(String className, ClassLoader classLoader) {
        if (classLoader == null)
            classLoader = GlobalVars.classLoader;
        try {
     ...
     *
     * <p>This combines calls to {@link #findMethodExact(Class, String, Object...)}
     *
     ...
     * to specify the callback that should be executed when the method is invoked.
     *
     * <p><b>Example</b>
     ...
     * findAndHookMethod("com.example.SomeClass", lpparam.classLoader, String.class, int.class, "com.example.MyClass", new XposedInterface.Hooker() {
     *   &#64;Override
     *   protected void beforeHookedMethod(XposedInterface.Chain chain) throws Throwable {
     *     String oldText = (String) chain.getArg(0);
     *     Log.d("MyModule", oldText);
     * <p>
     *     chain.getArg(0) = "test";
     *     chain.getArg(1) = 42; // auto-boxing is working here
     *     setBooleanField(chain.getArg(2), "great", true);
     * <p>
     *     // This would not work (as MyClass can't be resolved at compile time):
     *     //   MyClass myClass = (MyClass) chain.getArg(2);
     *     //   myClass.great = true;
     *   }
     * });
     * </pre>
    ...
    private static Class<?>[] getParameterClasses(ClassLoader classLoader, Object[] parameterTypesAndCallback) {
        ...
        for (int i = parameterTypesAndCallback.length - 1; i >= 0; i--) {
            ...
            if (type instanceof CakeHooker.Callback || type instanceof CakeHooker.BeforeCallback || type instanceof CakeHooker.AfterCallback || type instanceof XposedInterface.Hooker)
                continue;
    ...
    public static Object setAdditionalInstanceField(Object obj, String key, Object value) {
        ...
        synchronized (additionalFields) {
            objectFields = additionalFields.computeIfAbsent(obj, _ -> new HashMap<>());
        }
    ...
    private static ThreadLocal<AtomicInteger> getMethodDepthCounter(String method) {
        synchronized (sMethodDepth) {
            ...
            if (counter == null) {
                counter = ThreadLocal.withInitial(AtomicInteger::new);
    ...
    public static Object[] findParameterTypesFromClass(ClassLoader classLoader, String clazzName, String methodName) {
        for (Method method : findClassIfExists(clazzName, classLoader).getDeclaredMethods())
            if (method.getName().equals(methodName))
                return method.getParameterTypes();
        return null;
    }

    public static Method findMethodFromClass(ClassLoader classLoader, String clazzName, String methodName) {
        return findMethodFromClass(findClassIfExists(clazzName, classLoader), methodName);
    }

    public static Method findMethodFromClass(Class<?> clazz, String methodName) {
        if (clazz == null)
            return null;
        for (Method method : clazz.getDeclaredMethods())
            if (method.getName().equals(methodName))
    ...
    public static Class<?>[] findParameterTypesOrDefaultReturnClass(Class<?> clazz, String methodName, Object... parameter) {
        try {
            for (Method method : clazz.getDeclaredMethods()) {
                if (method.getName().equals(methodName)) {
                    Class<?>[] parameterTypes = method.getParameterTypes();
        ...
        for (Object originalParam : parameter) {
            classes.add(originalParam.getClass());
        }
        return classes.toArray(new Class<?>[0]);
    }

    public static Method findMethodBestMatchIfExists(Class<?> clazz, String methodName, Object... parameterTypesAndCallback) {
        if (clazz == null)
            return null;
    ...
    public static Object[] findConstructorParameterTypesOrDefault(Class<?> clazz, Object... parameter) {
        try {
